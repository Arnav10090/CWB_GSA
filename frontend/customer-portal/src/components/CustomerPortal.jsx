...
const CustomerPortal = () => {
  ...
  const [wantHelper, setWantHelper] = useState(true); // Default to true for backward compatibility
  ...
  const handleInputChange = (field, value) => {
    ...
    if (field === "driverAadhar") {
      // Only normalize to digits if ID type is aadhar
      if (formData.driverIdType === "aadhar" || !formData.driverIdType) {
        nextValue = normalizeAadharValue(value);
      } else {
        // For alphanumeric IDs, just uppercase and trim spaces/dashes
        nextValue = value.replace(/\s/g, "").replace(/-/g, "").toUpperCase();
      }
    }
    if (field === "helperAadhar") {
      // Only normalize to digits if ID type is aadhar
      if (formData.helperIdType === "aadhar" || !formData.helperIdType) {
        nextValue = normalizeAadharValue(value);
      } else {
        // For alphanumeric IDs, just uppercase and trim spaces/dashes
        nextValue = value.replace(/\s/g, "").replace(/-/g, "").toUpperCase();
      ...
      fieldNames.forEach((field) => {
        ...
        if (field === "driverAadhar") {
          if (!(driverExists && !driverChanged)) {
            const driverIdType = dataSource.driverIdType || "aadhar";
            const driverId = dataSource.driverAadhar?.replace(/\s/g, "").replace(/-/g, "").toUpperCase() || "";

            if (!driverId) {
              validationErrors.driverAadhar = "Driver ID number is required.";
            } else {
              // Validate based on ID type
              if (driverIdType === "aadhar") {
                const normalized = normalizeAadharValue(dataSource.driverAadhar);
                if (normalized.length !== 12) {
                  validationErrors.driverAadhar = "Aadhar must be exactly 12 digits.";
                }
              } else if (driverIdType === "voter_id") {
                if (!/^[A-Z]{3}\d{7}$/.test(driverId)) {
                  validationErrors.driverAadhar = "Voter ID must be in AAA1234567 format.";
                }
              } else if (driverIdType === "driving_license") {
                if (driverId.length < 13 || driverId.length > 16) {
                  validationErrors.driverAadhar = "Driving License must be 13-16 characters.";
                }
              } else if (driverIdType === "pan") {
                if (!/^[A-Z]{5}\d{4}[A-Z]$/.test(driverId)) {
                  validationErrors.driverAadhar = "PAN must be in AAAAA1234A format.";
                }
              }
            }
          }
        ...
        if (field === "helperAadhar") {
          ...
          if (!(helperExists && !helperChanged)) {
            ...
            if (!helperId) {
              validationErrors.helperAadhar = "Helper ID number is required.";
            } else {
              // Validate based on ID type
              if (helperIdType === "aadhar") {
                const normalized = normalizeAadharValue(dataSource.helperAadhar);
                if (normalized.length !== 12) {
                  validationErrors.helperAadhar = "Aadhar must be exactly 12 digits.";
                }
              } else if (helperIdType === "voter_id") {
                if (!/^[A-Z]{3}\d{7}$/.test(helperId)) {
                  validationErrors.helperAadhar = "Voter ID must be in AAA1234567 format.";
                }
              } else if (helperIdType === "driving_license") {
                if (helperId.length < 13 || helperId.length > 16) {
                  validationErrors.helperAadhar = "Driving License must be 13-16 characters.";
                }
              } else if (helperIdType === "pan") {
                if (!/^[A-Z]{5}\d{4}[A-Z]$/.test(helperId)) {
                  validationErrors.helperAadhar = "PAN must be in AAAAA1234A format.";
                }
              }
            }
          }
  ...
  const handleSaveDriver = async () => {
    ...
    if (!driverId) {
      validationError = "Driver ID is required";
    } else if (driverIdType === "aadhar") {
      const normalized = normalizeAadharValue(formData.driverAadhar);
      if (normalized.length !== 12) {
        validationError = "Driver Aadhar must be exactly 12 digits";
      }
    } else if (driverIdType === "voter_id") {
      if (!/^[A-Z]{3}\d{7}$/.test(driverId)) {
        validationError = "Voter ID must be in AAA1234567 format";
      }
    } else if (driverIdType === "driving_license") {
      if (driverId.length < 13 || driverId.length > 16) {
        validationError = "Driving License must be 13-16 characters";
      }
    } else if (driverIdType === "pan") {
      if (!/^[A-Z]{5}\d{4}[A-Z]$/.test(driverId)) {
        validationError = "PAN must be in AAAAA1234A format";
      }
    }

    if (validationError) {
      showPopupMessage(validationError, "warning");
      return;
    }
  ...
  const handleSaveHelper = async () => {
    ...
    let validationError = null;
    if (!helperId) {
      validationError = "Helper ID is required";
    } else if (helperIdType === "aadhar") {
      const normalized = normalizeAadharValue(formData.helperAadhar);
      if (normalized.length !== 12) {
        validationError = "Helper Aadhar must be exactly 12 digits";
      }
    } else if (helperIdType === "voter_id") {
      if (!/^[A-Z]{3}\d{7}$/.test(helperId)) {
        validationError = "Voter ID must be in AAA1234567 format";
  ...
  const handleAddDriver = async () => {
    ...
    if (!driverId) {
      errors.driverAadhar = "Driver ID is required";
    } else if (driverIdType === "aadhar") {
      const normalized = normalizeAadharValue(formData.driverAadhar);
      if (normalized.length !== 12) {
        errors.driverAadhar = "Aadhar must be exactly 12 digits";
      }
    } else if (driverIdType === "voter_id") {
      if (!/^[A-Z]{3}\d{7}$/.test(driverId)) {
        errors.driverAadhar = "Voter ID must be in AAA1234567 format";
      }
    } else if (driverIdType === "driving_license") {
      if (driverId.length < 13 || driverId.length > 16) {
        errors.driverAadhar = "Driving License must be 13-16 characters";
      }
    } else if (driverIdType === "pan") {
      if (!/^[A-Z]{5}\d{4}[A-Z]$/.test(driverId)) {
        errors.driverAadhar = "PAN must be in AAAAA1234A format";
      }
    }
  ...
  const handleAddHelper = async () => {
    ...
    // Validate ID based on type
    if (!helperId) {
      errors.helperAadhar = "Helper ID is required";
    } else if (helperIdType === "aadhar") {
      const normalized = normalizeAadharValue(formData.helperAadhar);
      if (normalized.length !== 12) {
        errors.helperAadhar = "Aadhar must be exactly 12 digits";
      }
    } else if (helperIdType === "voter_id") {
      if (!/^[A-Z]{3}\d{7}$/.test(helperId)) {
        errors.helperAadhar = "Voter ID must be in AAA1234567 format";
      }
    } else if (helperIdType === "driving_license") {
      if (helperId.length < 13 || helperId.length > 16) {
        errors.helperAadhar = "Driving License must be 13-16 characters";
      }
    } else if (helperIdType === "pan") {
      if (!/^[A-Z]{5}\d{4}[A-Z]$/.test(helperId)) {
        errors.helperAadhar = "PAN must be in AAAAA1234A format";
      }
    }
  ...
  const handleDriverModalSave = async (driverData) => {
    // Conditionally normalize based on ID type
    const requestedUid = (driverData.idType === "aadhar" || !driverData.idType)
      ? normalizeAadharValue(driverData.aadhar)
      : driverData.aadhar.replace(/\s/g, "").replace(/-/g, "").toUpperCase();
    ...
    const localDriver = {
      ...
      idType: driverData.idType || "aadhar",
    };
    ...
    setFormData((prev) => ({
      ...
      driverIdType: driverData.idType || "aadhar",
    }));
  ...
  const handleHelperModalSave = async (helperData) => {
    // Conditionally normalize based on ID type
    const requestedUid = (helperData.idType === "aadhar" || !helperData.idType)
      ? normalizeAadharValue(helperData.aadhar)
      : helperData.aadhar.replace(/\s/g, "").replace(/-/g, "").toUpperCase();
    ...
    const localHelper = {
      ...
      idType: helperData.idType || "aadhar",
    };
    ...
    setFormData((prev) => ({
      ...
      helperIdType: helperData.idType || "aadhar",
    }));
  ...
  const handleDriverSelect = async (driver) => {
    ...
    setFormData((prev) => ({
      ...
      driverIdType: driver.idType || "aadhar",
    }));
  ...
  const handleHelperSelect = async (helper) => {
    ...
    setFormData((prev) => ({
      ...
      helperIdType: helper.idType || "aadhar",
    }));
          ...
          if (normalizedVehicleNumber) {
            ...
            if (!vehicleCreated) {
              ...
              if (Array.isArray(drivers) && drivers.length > 0) {
                ...
                if (driver.idType) {
                  updates.driverIdType = driver.idType;
                }
                setSavedDriverData(driver);
              ...
              if (Array.isArray(helpers) && helpers.length > 0) {
                ...
                if (helper.idType) {
                  updates.helperIdType = helper.idType;
                }
                setSavedHelperData(helper);
  ...
  const makeDemoQr = (poDriverVehicleTaggingId) => {
    if (!poDriverVehicleTaggingId) {
      return "";
    }
    const payload = {
      id: poDriverVehicleTaggingId,
    };
  ...
  const handleDownloadQR = async () => {
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        ...
        link.href = url;
        link.download = `QR-${successData?.vehicleNumber || 'code'}.png`;
      ...
      // Fallback to downloading just the QR image if html2canvas fails  
      const qrImage = successData.qrCodeImage || makeDemoQr(successData.poDriverVehicleTaggingId);
      const link = document.createElement('a');
      link.href = qrImage;
      link.download = `QR-${successData?.vehicleNumber || 'code'}.png`;
      document.body.appendChild(link);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccessData({
        ...
        driverPhone: formData.driverPhone,
        poDriverVehicleTaggingId: submission?.poDriverVehicleTaggingId,
        qrCodeImage:
          submission?.qrCodeImage || makeDemoQr(submission?.poDriverVehicleTaggingId),
      });
  ...
  if (successData) {
    const qrCodeImage =
      ...
      makeDemoQr(successData.poDriverVehicleTaggingId);
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 px-4 py-10">
        ...
        {showPopup && (
          <div className="fixed right-6 top-28 z-50 w-full max-w-sm rounded-xl bg-white shadow-lg">
            <div
              className={`flex items-start gap-3 p-4 ${popupVariant === "warning"
        ...
        <div className="mx-auto max-w-4xl">
          <div className="rounded-2xl bg-white p-8 shadow-xl">
            ...
            <div className="mt-8 grid gap-8 lg:grid-cols-2">
              <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm" id="qr-card-to-download">
                <div className="flex justify-center">
                  <div className="rounded-2xl border-4 border-gray-100 bg-white p-4">
                    <img
                      src={qrCodeImage}
                      alt="Entry QR Code"
                ...
                <div className="mt-6 text-center">
                  ...
                  <p className="mt-4 text-sm text-gray-600">QR code text:</p>
                  <p className="text-xl font-bold text-gray-900">
                    {successData.poDriverVehicleTaggingId || "—"}
                  </p>
                </div>
              </div>
              <div className="flex flex-col justify-between rounded-2xl border border-blue-100 bg-blue-50 p-6">
                ...
                <div style={{
                  ...
                  <style>{`
                    @keyframes glow {
                      ...
                      50% {
                        text-shadow: 0 0 20px rgba(34, 197, 94, 0.8), 0 0 30px rgba(34, 197, 94, 0.5), 0 0 40px rgba(34, 197, 94, 0.3);
                      }
                    }
                  `}</style>
                  <h2 style={{
                    ...
                    📥 Download QR and send the QR to Driver! 🚗
                  </h2>
                </div>
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <button
                    ...
                    className="flex-1 inline-flex items-center justify-center gap-2 rounded-xl bg-green-600 px-4 py-3 text-sm font-semibold text-white transition-all duration-200 hover:bg-green-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-green-500 focus-visible:ring-offset-2"
                  >
            ...
            <nav className="grid gap-3 sm:grid-cols-3">
              {steps.map((step) => {
                ...
                return (
                  <div
                    key={step.id}
                    className={`rounded-xl border px-4 py-3 transition-all duration-200 ${isActive
            ...
            <form className="space-y-8">
              ...
              {currentStep === 1 && (
                <Step2DriverInfo
                  ...
                  loadingVehicleData={loadingVehicleData}
                  wantHelper={wantHelper}
                  onWantHelperChange={setWantHelper}
                />
              )}
