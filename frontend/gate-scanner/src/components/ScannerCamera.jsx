...
export default function ScannerCamera({ active, onDetected, onError }) {
  ...
  const describeGetUserMediaError = useCallback((err) => {
    const name = err?.name || '';
    const msg = (err?.message || '').toLowerCase();

    if (name === 'NotAllowedError' || name === 'PermissionDeniedError' || msg.includes('permission') || msg.includes('denied')) {
      return 'Camera permission denied. Please allow camera access in your browser site settings and try again.';
    }
    if (name === 'NotFoundError' || name === 'DevicesNotFoundError') {
      return 'No camera device found.';
    }
    if (name === 'NotReadableError' || name === 'TrackStartError') {
      return 'Camera is already in use by another application or tab. Close other apps using the camera and try again.';
    }
    if (name === 'OverconstrainedError' || name === 'ConstraintNotSatisfiedError') {
      return 'Requested camera constraints are not supported on this device. Trying a fallback camera mode may help.';
    }
    if (name === 'SecurityError') {
      return 'Camera access is blocked due to an insecure context. Use HTTPS (or localhost) and try again.';
    }
  ...
  const startCamera = useCallback(async () => {
    try {
      ...
      if (!navigator?.mediaDevices?.getUserMedia) {
        ...
        onError?.('Camera API not available in this browser. Please use a modern browser (Chrome/Edge/Safari) and try again.');
        return;
      }

      if (!window.isSecureContext) {
        setCameraStatus('error');
        onError?.('Camera access requires HTTPS (or localhost). Open the app on https:// or http://localhost and try again.');
        return;
      }
      ...
      try {
        ...
        if (perm?.state === 'denied') {
          setCameraStatus('error');
          onError?.('Camera permission is blocked for this site. Allow it in browser site settings and reload.');
          return;
        }
      } catch {
        // ignore: Permissions API not supported or camera permission not queryable
      }
      ...
      try {
        ...
        // Some devices/browsers don't support facingMode/environment well; fallback to any camera.
        try {
      ...
      if (msg.includes('play() request was interrupted')) {
        ...
        onError?.(describeGetUserMediaError(e));
      }
    }
  }, [scanFrame, onError, stopCamera, describeGetUserMediaError]);
