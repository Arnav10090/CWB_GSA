...
class DriverHelperViewSet(viewsets.ModelViewSet):
    ...
    def validate_or_create(self, request):
        ...
        id_type = serializer.validated_data.get('idType', 'aadhar')
        ...
        id_type_label = id_type_names.get(id_type, 'ID number')
        ...
        try:
                ...
                if existing_by_uid:
                    ...
                    if existing_by_uid.phoneNo == phone_no:
                        ...
                        if existing_by_uid.name.lower() == name.lower():
                            ...
                            return Response({
                                "error": f"{id_type_label} is already registered with name '{existing_by_uid.name}'. Please verify the details."
                            }, status=status.HTTP_400_BAD_REQUEST)
                    else:
                        # Same UID but different phone
                        return Response({
                            "error": f"{id_type_label} is already registered with a different phone number. Please verify the {id_type_label}."
                        }, status=status.HTTP_400_BAD_REQUEST)
            ...
            if not uid:
                return Response({
                    "error": f"{id_type_label} is required to create a new driver/helper"
                }, status=status.HTTP_400_BAD_REQUEST)
            ...
            instance = DriverHelper.objects.create(
                ...
                type=driver_type,
                language=language,
                idType=serializer.validated_data.get('idType', 'aadhar'),
                isBlacklisted=False,
                rating=None
            )
